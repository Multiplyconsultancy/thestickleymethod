...
const { isBase44Eligible } = require('./base44')
...
const OWNED = /^(customer-|spent-|churn-|plan-|cancelling$|has-tsm$|has-nightfall-97$|has-nightfall-legacy$|has-baby-ai$)/
 ...
 */
async function applyAuthoritative(email, person, { create = true, name = '', contact: given } = {}) {
  ...
  if (isBase44Eligible(contact || {})) want.push('base44-eligible')
...
async function applyFromEvent(email, { tags = [], name = '' } = {}) {
  ...
  if (isBase44Eligible(contact || {})) add.push('base44-eligible')
