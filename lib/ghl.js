...
/* Dunning idempotency lives in two CONTACT CUSTOM FIELDS, not in tags.
   It used to be a tag per failed payment, `dunning-<payment id>`. The
   behaviour was right (one failure, one email; a later unrelated failure
   emails again) but the storage was wrong: a GHL tag is location-wide, so
   a private note about one person's card became a permanent entry in the
   tag picker for all 55,000 contacts. 29 of the location's 97 tags were
   these. Custom fields are per-contact and cost the namespace nothing.

   Ids are from this location, created 12 Aug 2026. Looked up by id, with
   a fieldKey fallback so a re-created field does not silently disable
   the check and re-email everyone. */
const DUNNING_FIELD_ID = '2UUfBlcnhbh04tnSomXF'   // contact.last_dunning_payment_id
const DUNNING_AT_FIELD_ID = 'DWTijmEgEYjPo7kp4jfw' // contact.last_dunning_at

/* A second failure within this window is not emailed again. The tag scheme
   could not express this at all: every distinct payment id looked new, so
   a card retrying three times in a week meant three emails. */
const DUNNING_COOLDOWN_DAYS = 14

function customFieldValue(contact, id, key) {
  const fields = contact?.customFields || []
  const hit = fields.find(f => f.id === id) || fields.find(f => f.key === key || f.fieldKey === key)
  return hit ? String(hit.value ?? hit.field_value ?? '') : ''
}

/**
 * Payment-failed recovery email, once per failed payment.
 *
 * Skips when this exact payment has already been emailed about, and when
 * anything was emailed within the cool-down, so a card retrying several
 * times in a week produces one email rather than one per attempt.
 */
async function sendDunningEmail(email, name, productLabel, paymentId) {
  ...
  try {
    ...
    /* Base44 buyers are off email entirely (standing rule, 2026-09-17): the
       only email they ever get is the one delivering their recorded video
       audit. A failed TSM payment on a Base44 buyer is a real problem, but it
       gets chased by message, not by an automated email. */
    const tags = (contact.tags || []).map(t => String(t).toLowerCase())
    if (tags.some(t => t.startsWith('base44-sale-'))) {
      return { ok: true, skipped: true, reason: 'base44_buyer_no_email' }
    }

    // Same payment, already handled.
    const lastId = customFieldValue(contact, DUNNING_FIELD_ID, 'contact.last_dunning_payment_id')
    if (lastId && lastId === String(paymentId)) return { ok: true, skipped: true, reason: 'same_payment' }

    // Different payment, but too soon after the last one.
    const lastAt = customFieldValue(contact, DUNNING_AT_FIELD_ID, 'contact.last_dunning_at')
    const lastMs = lastAt ? Date.parse(lastAt) : NaN
    if (Number.isFinite(lastMs) && Date.now() - lastMs < DUNNING_COOLDOWN_DAYS * 86400000) {
    ...
    // Written only AFTER the send succeeded, so a failure retries next tick.
    try {
      await ghl(`/contacts/${encodeURIComponent(contactId)}`, 'PUT', {
        customFields: [
          { id: DUNNING_FIELD_ID, value: String(paymentId) },
          { id: DUNNING_AT_FIELD_ID, value: new Date().toISOString() },
        ],
      })
    } catch (e) { console.error('[ghl] dunning field write failed:', e.message) }
