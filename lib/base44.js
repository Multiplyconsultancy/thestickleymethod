...
const DIAL = {
  ...
  '972': 'IL', '973': 'BH', '974': 'QA', '977': 'NP', '994': 'AZ', '998': 'UZ',
}

/* The country list Luca supplied was cut off at Switzerland, so this is
   the confirmed part of it. Anything added later goes here and the next
   reconcile picks the new people up on its own. */
const ELIGIBLE_COUNTRIES = new Set([
  'US/CA', 'UK', 'AU', 'NZ', 'AT', 'BE', 'DK', 'FR', 'DE', 'IE',
  'IT', 'LU', 'NL', 'NO', 'PL', 'ES', 'SE', 'CH',
])
 ...
 */
/* GHL stores a real ISO country code on the contact, and it beats
   guessing from a phone number. Ignoring it marked 677 UAE contacts as
   Base44-eligible, because +97555555558 matches no dialling code in the
   table and "unknown" was being read as "fine". The country field is
   evidence; an unparseable phone is not. */
const ISO_ELIGIBLE = new Set([
  'US', 'CA', 'GB', 'UK', 'AU', 'NZ', 'AT', 'BE', 'DK', 'FR',
  'DE', 'IE', 'IT', 'LU', 'NL', 'NO', 'PL', 'ES', 'SE', 'CH',
])
...
function isBase44Eligible(contact) {
  ...
  /* Country first, when we have one. It is stated rather than inferred. */
  if (/^[A-Z]{2}$/.test(country)) return ISO_ELIGIBLE.has(country)
