...
async function fetchAllMemberships(onProgress) {
  const out = []
  for (let page = 1; page <= PAGE_GUARD; page++) {
    const j = await whop(`${V2}/memberships?per=${PER}&page=${page}`)
    const rows = j.data || []
    out.push(...rows)
    if (onProgress) onProgress(out.length, j.pagination?.total_count ?? null)
    const totalPages = Number(j.pagination?.total_page ?? 0)
    if (rows.length < PER || (totalPages && page >= totalPages)) break
    if (page === PAGE_GUARD) console.warn('[whop] memberships hit the page guard')
    await sleep(110)
  }
  return out
}
