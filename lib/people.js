...
function blank(email) {
  return {
    ...
    spend: 0, lastPaidAt: null, daysSinceLastPayment: null,
...
function buildTruthTable({ memberships = [], payments = [], fbCustomers = [], fbSubscribers = [],
  ...
  for (const m of memberships) {
    const email = normEmail(whop.emailOf(m))
    if (!email) continue
    const p = get(email)
    p.whop.known = true
  ...
  for (const pay of payments) {
    const email = normEmail(pay?.user?.email)
    if (!email) continue
    ...
    if (pay.paid_at) p.whop.lastPaidAt = newer(p.whop.lastPaidAt, pay.paid_at)
  ...
  for (const p of people.values()) {
    ...
    /* CHURN IS MEASURED FROM WHEN ACCESS ENDED, NOT FROM THE LAST PAYMENT.
       Someone who paid on 7 July and lost access on 6 August churned last
       week, not five weeks ago. Banding on payment date put 19% of 4,962
       expired memberships in the wrong band, always ageing the freshest
       and most recoverable people out of reach. Falls back to the payment
       date only when no expiry is recorded, which is the Fanbasis case. */
    p.accessEndedAt = p.whop.accessEndsAt || p.lastPaidAt
    p.daysSinceAccessEnded = daysSince(p.accessEndedAt)
