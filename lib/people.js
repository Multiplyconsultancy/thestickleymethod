...
function blank(email) {
  return {
    email,
    name: '', phone: '', country: '',
    whop: { known: false, paying: false, everPaid: false, spend: 0, refunded: 0,
...
function buildTruthTable({ memberships = [], payments = [], fbCustomers = [], fbSubscribers = [],
  ...
  for (const p of people.values()) {
    ...
    p.recencyBand = p.segment === 'churned' ? recencyBand(p.daysSinceLastPayment) : null
 ...
 */
function segmentFor(ghlEmail, table) {
  const email = normEmail(ghlEmail)
  if (!email) return { segment: null, reason: 'no_email' }
  const p = table.get(email)
  if (!p) return { segment: 'cold', person: null }
  return { segment: p.segment, person: p }
}

module.exports = {
  normEmail, daysSince, recencyBand, ltvBand,
