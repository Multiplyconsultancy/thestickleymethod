...
const BOARDS = [
  ...
  /* ── Base44 ────────────────────────────────────────────────────────
     ...
     EVERY ELIGIBLE PERSON GETS A CARD, not just the ones who opted in.
     Without the eligible pool sitting in column one there is no
     denominator, and a funnel board with no denominator cannot show you
     a conversion rate, only a list of survivors. The Eligible column is
     large (18,826 on "Everyone else") and that is fine: GHL loads a
     board a stage at a time, and the columns a setter actually works
     are the small ones to its right.

     THE MACHINE OWNS TWO STAGES, Eligible and Opted in, because those
     are the only two facts we hold: a dialling code, and a hit on our
     own opt-in endpoint. Called, Signed up monthly, Signed up yearly and
     Lost are human-owned. Base44 sends no signal back, so a setter
     marking the card is the only record that any of it happened, and
     nothing automated may move a card out of those columns.
     ...
     is never lost because somebody bought something. */
  {
    ...
    family: 'base44',
    belongs: (p, tags) => has(tags, 'base44-eligible') && has(tags, 'customer-active'),
  ...
  },
  {
    ...
    family: 'base44',
    belongs: (p, tags) => has(tags, 'base44-eligible') && has(tags, 'customer-churned'),
    machine: ['Eligible', 'Opted in'],
    stage: (p, tags) => (has(tags, 'base44-opted-in') ? 'Opted in' : 'Eligible'),
  },
  {
    ...
    family: 'base44',
    belongs: (p, tags) => has(tags, 'base44-eligible') &&
