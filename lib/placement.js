...
const BOARDS = [
  ...
  },
  {
    ...
    /* SPLIT BY AGE, NOT BY SPEND.
       7,851 people is too many to work as one column. Spend cannot split
       them: 82% sit in the bottom two bands and the top three hold 173
       people between them, so you get one bucket of 6,473 and three that
       barely exist. Age splits usably, and it predicts the thing that
       decides a phone call, which is whether they still remember Baby.
       Callers work top down. */
    machine: ['0 to 30 days', '31 to 90 days', '91 to 180 days',
              '181 to 365 days', '1 to 2 years', 'Opted in'],
    stage: (p, tags) => {
      if (has(tags, 'base44-opted-in')) return 'Opted in'
      if (has(tags, 'churn-0-30d')) return '0 to 30 days'
      if (has(tags, 'churn-31-90d')) return '31 to 90 days'
      if (has(tags, 'churn-91-180d')) return '91 to 180 days'
      if (has(tags, 'churn-181-365d')) return '181 to 365 days'
      if (has(tags, 'churn-1-2y')) return '1 to 2 years'
      return null                       // older than 2 years: no card
    },
  },
  {
