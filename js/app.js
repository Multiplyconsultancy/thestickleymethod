...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function drawGlobeFrame(idx) {
    var img = globeFrames[idx];
    ...
    globeCtx.drawImage(img, sx, 0, crop, crop, 0, 0, cw, ch);
  }
  ...
  if (globeCanvas && globeCtx) {
    var globeObs = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        ...
        if (globeVisible && globeLoaded >= 10) {
          startGlobeLoop();
    ...
    for (var gi = 0; gi < GLOBE_COUNT; gi++) {
      (function (gidx) {
        var img = new Image();
        img.onload = function () {
          globeFrames[gidx] = img;
          globeLoaded++;
          if (gidx === 0) drawGlobeFrame(0);
          if (globeLoaded === 10 && globeVisible) startGlobeLoop();
        };
        img.src = 'globe-frames/globe_' + padGlobe(gidx) + '.png';
      })(gi);
    }
  }
